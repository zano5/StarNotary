...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    let starId = 6;
    await instance.createStar('Zain', starId, {from: accounts[0]});

    assert.equal(await instance.tokenIdToStarInfo.call(starId), 'Zain');
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Pluto', starId1, {from: accounts[0]});
    await instance.createStar('Jupiter', starId2, {from: accounts[1]});
...
it('Transfer a star', async() => {
    // 1. create a Star with different tokenId
    // 2. use the transferStar function implemented in the Smart Contract
    // 3. Verify the star owner changed.
    let instance = await StarNotary.deployed();
    let starId = 9;
    await instance.createStar('Neptune', starId, {from: accounts[0]});
